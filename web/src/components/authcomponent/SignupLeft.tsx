...
export function SignUpLeft() {
    ...
    return <div className="w-[50%]">
        ...
        <div className="py-2"><EntryBoxComponent value={setUsername} title="Username" type="text" /></div>
        <div className="py-2"><EntryBoxComponent title="Email" type="text" value={setEmail} /></div>
        <div className="py-2"><EntryBoxComponent title="Password" type="password" value={setPassword} /></div>
        {(loading === false) ? <CustomButton onTap={() => {
            signUpUser()
        }} title="Sign up" /> : <LoadingButton size="w-[100%]" />}
        <div className="flex justify-center">
            <p>Already have account ? </p>
            <p className="font-semibold px-1 hover:cursor-pointer" onClick={() => { navigate('/auth/signin') }}>
                Sign In
            </p>
        </div>
